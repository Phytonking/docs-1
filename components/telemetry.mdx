...
For teams looking to add AARM-aligned telemetry capture with minimal effort, the [mcp-observe](/guides/mcp-observe) SDK provides one-line instrumentation. It automatically captures every LLM and MCP tool call and streams structured events to a real-time dashboard.
...
mcp-observe captures tool name, arguments, results, errors, model info, and token usage — aligning with the telemetry event types described above. It also supports [sessions and traces](/guides/mcp-observe#sessions-and-traces) for grouping related events.

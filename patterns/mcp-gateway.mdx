...
## Quick Start with mcp-observe Proxy CLI

The [mcp-observe](/guides/mcp-observe) SDK includes a proxy CLI that implements the MCP Gateway pattern with zero code changes. It wraps any MCP server as a transparent stdio proxy, intercepting all JSON-RPC `tools/call` messages:
...
The proxy captures tool name, arguments, and results for every call, then forwards everything unchanged to the downstream server. This provides immediate observability while you build out full AARM enforcement.
...
<CardGroup cols={2}>
  ...
  <Card title="mcp-observe SDK" icon="eye" href="/guides/mcp-observe">
    One-line observability for MCP tool calls
  </Card>
</CardGroup>
